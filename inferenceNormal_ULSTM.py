...
def run_inference():
    ...
    current_subseq_name = None

    print(f"Iniciando inferencia...")
    batch_count = 0
    with torch.no_grad():
        for imgs, labels, target_paths, anomaly_flags in tqdm(dataloader):
            sample_path = target_paths[0][0]
            base_name = sample_path.split("/")[-2]
            is_anomaly = anomaly_flags[0].item()

            if is_anomaly == 1:
                was_in_anomaly = True
                continue
                
            if base_name != last_base_name or was_in_anomaly:
                if base_name != last_base_name:
                    subseq_counter = 1
                else:
                    subseq_counter += 1
                current_subseq_name = f"{base_name}_{subseq_counter}"
                results[current_subseq_name] = {'psnr': [], 'ssim': [], 'lpips': []}
                was_in_anomaly = False
            ...
            for b in range(predictions.size(0)):
                ...
                results[current_subseq_name]['psnr'].append(seq_avg_psnr)
                results[current_subseq_name]['ssim'].append(seq_avg_ssim)
    ...
    with open(csv_filename, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['seq_name', 'psnr_evolution', 'ssim_evolution'])

        for name, metrics in results.items():
            writer.writerow([name, metrics['psnr'], metrics['ssim']])
